...
$postfix = '';
global $arSite;
if(\Bitrix\Main\Config\Option::get("aspro.mshop", "HIDE_SITE_NAME_IN_TITLE", "N")=="N")
...
$oCrossSales = new \Aspro\MShop\CrossSales($ElementID, $arParams);
...
if($arNeedSelect){
	...
	$arElement = array_merge(
		$arElement,
		CMShopCache::CIBLockElement_GetList(
			array(
				'CACHE' => array(
					'MULTI' => 'N',
					'TAG' => CMShopCache::GetIBlockCacheTag($arParams['IBLOCK_ID'])
				)
			),
			array(
				'ID' => $ElementID,
...
if(!$arRules['EXPANDABLES']){
	...
	if($arExpValues)
	{
		$arItems = CMshopCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CMshopCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arExpValues), false, false, array("ID"));
		if($arItems)
		{
...
if(!$arRules['ASSOCIATED']){
	$arAssociated=CMshopCache::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID, array("CACHE" => array("TAG" =>CMshopCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("CODE" => "ASSOCIATED"));
	if($arAssociated)
	{
		$arItems = CMshopCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CMshopCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arAssociated), false, false, array("ID"));
		if($arItems)
		{

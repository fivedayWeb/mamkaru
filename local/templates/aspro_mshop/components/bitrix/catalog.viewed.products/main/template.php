							...
							<li class="catalog_item" id="<?=$strMainID;?>">
								<div class="image_wrapper_block">
									<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb">
										<?if($arItem["PROPERTIES"]["HIT"] && $arItem["PROPERTIES"]["HIT"]["VALUE_XML_ID"]){?>
											<div class="stickers">
									...
									<div class="cost prices clearfix">
										<?if($arItem["OFFERS"]):?>
											...
											if($minPrice["VALUE"]>$minPrice["DISCOUNT_VALUE"]){?>
												<div class="price"><?=GetMessage("CATALOG_FROM");?> <span class="values_wrapper"><?=$minPrice["PRINT_DISCOUNT_VALUE"];?></span>
											...
											<?foreach($arItem["PRICES"] as $key => $arPrice):?>
												<?if($arPrice["CAN_ACCESS"]):
													...
													<?if($arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"]):?>
														<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_VALUE"];?>"><span class="values_wrapper"><?=CCurrencyLang::CurrencyFormat(($arPrice["DISCOUNT_VALUE"]), $arPrice["CURRENCY"]);?></span>
														<?if (($arParams["SHOW_MEASURE"]=="Y") && $strMeasure){?>
															<span class="price_measure">/<?=$strMeasure?></span>
														<?}?>
														</div>
														<?if($arParams["SHOW_OLD_PRICE"]=="Y"):?>
															<div class="price discount" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["VALUE"];?>">
																<span class="values_wrapper"><?=CCurrencyLang::CurrencyFormat(($arPrice["VALUE"]), $arPrice["CURRENCY"]);?></span>
															</div>
														<?endif;?>
														<?if($arParams["SHOW_DISCOUNT_PERCENT"]=="Y"){?>
															<div class="sale_block">
																...
																<?if($percent && $percent<100){?>
																	<div class="value">-<?=$percent;?>%</div>
																<?}?>
																<div class="text"><?=GetMessage("CATALOG_ECONOMY");?> <span data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_DIFF"];?>"><?=CCurrencyLang::CurrencyFormat(($arPrice["DISCOUNT_DIFF"]), $arPrice["CURRENCY"]);?></span></div>
																<div class="clearfix"></div>
															</div>
														<?}?>
													<?else:?>
														<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["VALUE"];?>"><span class="values_wrapper"><?=CCurrencyLang::CurrencyFormat(($arPrice["VALUE"]), $arPrice["CURRENCY"]);?></span>
														<?if (($arParams["SHOW_MEASURE"]=="Y") && $strMeasure){?>
															<span class="price_measure">/<?=$strMeasure?></span>
														<?}?>
														</div>

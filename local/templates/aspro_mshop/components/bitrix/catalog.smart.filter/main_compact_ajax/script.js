$(document).ready(function(){
	...
	$(document).on('click', '.inner_wrapper .bx_filter_button_box .bx_filter_search_button', function(e){
		if(window.matchMedia('(max-width: 767px)').matches){
			if(!$('.adaptive_filter.compact_view').length){
	...
	$.ajax({
		...
		success: function(html){
			...
			if(reset && (!$('#wrapInlineFilter').length || !$('#wrapInlineFilter').is(':empty')))
			{
				...
				$('#right_block_ajax').html(html);
			}
			else
			{
				var itemsHtml = $(html).find('.ajax_load').html();

				$('.middle > .container .ajax_load').html(itemsHtml);

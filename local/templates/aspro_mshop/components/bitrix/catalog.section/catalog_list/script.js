...
if (!window.JCCatalogSectionOnlyElement)
{
	...
	window.JCCatalogSectionOnlyElement.prototype = {
		...
		setPriceAction: function()
		{
			...
			if(arMShopOptions['THEME']['SHOW_TOTAL_SUMM'] != 'N' && typeof(this.currentPrices[this.currentPriceSelected]) != 'undefined' && typeof(this.currentPrices[this.currentPriceSelected].PRICE) !== 'undefined')
			{
...
window.JCCatalogSection = function (arParams)
{
	...
	if ('object' === typeof arParams)
	{
		...
		this.skuDetailID = arParams.SKU_DETAIL_ID;
				...
				if(obPrices[j])
				{
					...
					strPrice += '<div class="offers_price">';
						...
						if(this.offers[this.offerNum].MAX_QUANTITY > 0)
						{
							...
							strPrice += '<span class="values_wrapper not_qt">Цену уточняйте<br>у менеджера</span>';
						}
						//poulhouse
						//strPrice += '<span class="values_wrapper">'+getCurrentPrice(obPrices[j].DISCOUNT_VALUE, obPrices[j].CURRENCY, obPrices[j].PRINT_DISCOUNT_VALUE)+'</span>';
						if(measure)

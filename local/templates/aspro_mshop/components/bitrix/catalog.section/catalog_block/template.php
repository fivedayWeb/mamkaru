				...
				<div class="catalog_item item_wrap <?=(($_GET['q'])) ? 's' : ''?>" id="<?=$arItemIDs["strMainID"];?>">
					<div>
						...
						<div class="item_info main_item_wrapper <?=$arParams["TYPE_SKU"]?>">
							...
							<?=$arQuantityData["HTML"];?>
							<div class="cost prices test clearfix">
								...
								<?if( $arItem["OFFERS"]){?>
									...
									<?/*poulhouse*/if($_GET['q']){ ?>
										...
										<script>
										if($('.catalog_item.s .item-stock').find('span.order')){$('.catalog_item.s .item-stock').find('span.order').parent().parent().find('div.cost.prices').html('<div class="price"><div class="offers_price_wrapper"><div class="offers_price"><span class="values_wrapper not_qt">Цену уточняйте<br>у менеджера</span></div></div></div>');}</script>
								...
								<?}else{?>
									...
									/*poulhouse*/if($_GET['q']){ ?>
										...
										<script>if($('.catalog_item.s .item-stock').find('span.order')){$('.catalog_item.s .item-stock').find('span.order').parent().parent().find('div.cost.prices').html('<div class="price"><div class="offers_price_wrapper"><div class="offers_price"><span class="values_wrapper not_qt">Цену уточняйте<br>у менеджера</span></div></div></div>');}</script>
									<? } else {?>

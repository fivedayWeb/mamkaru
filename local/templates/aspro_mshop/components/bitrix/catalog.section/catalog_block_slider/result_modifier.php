		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$arOneRow = array(
						...
						'SHOW_DISCOUNT_TIME_EACH_SKU' => ($arParams['SHOW_DISCOUNT_TIME_EACH_SKU'] ? $arParams['SHOW_DISCOUNT_TIME_EACH_SKU'] : 'N'),
				...
				if(empty($arItem['OFFERS_PROP']))
				{
					...
					if($arParams['USE_PRICE_COUNT'] == 'Y')
					{
						...
						foreach($arItem['OFFERS'] as $keyOffer => $arOffer)
						{
							...
							$arItem['OFFERS'][$keyOffer] = array_merge($arOffer, CMShop::formatPriceMatrix($arOffer));
						}
					}

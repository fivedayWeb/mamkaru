			...
			<div class="stores news">
							...
							<li class="item">
								<div class="wrapp_block">
									...
									<?if(strlen($arItem['PROPERTIES']["ADDRESS"]['VALUE']) && in_array('ADDRESS', $arParams["PROPERTY_CODE"])):?>
										...
										<div class="clear"></div>
									<?endif;?>
									<?if($arItem['PROPERTIES']["PHONE"]['VALUE'] && in_array('PHONE', $arParams["PROPERTY_CODE"])):?>
										<div class="store_text">
											...
											<?
													...
													foreach($arItem['PROPERTIES']["PHONE"]['VALUE'] as $phone){
														$phonesHTML .= '<div class="phone"><a rel="nofollow" href="tel:'.str_replace([" ","-","(",")"], "", $phone).'">'.$phone.'</a></div>';
													}
												?>
												<?=$phonesHTML;?>
											</div>
				...
				<div class="all_map">
					<a href="<?=$listurl?>" class="wrapp_block">
						<div class="icon"></div>
						<div class="text">
							<?$APPLICATION->IncludeFile(SITE_DIR."include/front_stores_title.php", Array(), Array( "MODE" => "text", "NAME" => GetMessage("FRONT_STORES"), )); ?>
						</div>
					</a>
				</div>
			</div>

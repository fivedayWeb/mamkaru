...
<?if($arResult['ITEMS']):?>
	<div class="content_wrapper_block <?=$templateName;?> <?=$arParams['SIZE_IN_ROW'] > 1 ? 'with-border' : ''?>">
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL'] || ($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW'])):?>
							...
							<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
								...
								<div class="block-links">									
									...
									<?if($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW']):?>
										<span><span class="btn btn-transparent-border-color btn-sm animate-load" data-event="jqm" data-param-form_id="REVIEW" data-name="send_review" title="<?=$arParams['TITLE_ADD_REVIEW'] ;?>"><?=CMShop::showIconSvg("resume colored", SITE_TEMPLATE_PATH."/images/svg/leaveareview.svg", "", "", true, false);?></span></span>
									<?endif;?>
								</div>
							<?endif;?>
						</div>
			...
			<?else:?>
				<div class="top_block">
					...
					<?endif;?>
					<?if($arParams['SHOW_ADD_REVIEW'] == 'Y' && $arParams['TITLE_ADD_REVIEW']):?>
						<span class="pull-right reviews"><span class="muted dark_link animate-load" data-event="jqm" data-param-form_id="REVIEW" data-name="send_review"><?=CMShop::showIconSvg("resume", SITE_TEMPLATE_PATH."/images/svg/leaveareview.svg", "", "", true, false);?><span><?=$arParams['TITLE_ADD_REVIEW'] ;?></span></span></span>
					<?endif;?>
				</div>
			<?endif;?>
		<?endif;?>
		...
		<div class="item-views reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>">
			...
			<?else:?>
				<div class="flexslider with-init<?=(!$bOneItem ? ' shadow' : '');?>" data-plugin-options='{<?if($bOneItem):?>"smoothHeight1": true,<?endif;?> "directionNav": true, "useCSS": true, "controlNav" :true, "itemMargin": 20, "animationLoop": true, "slideshow": false, <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "counts": [<?=$col;?>,<?=$col;?>, 1, 1]}'>
			<?endif;?>
						...
						<li class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
							<div class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
											...
											<div class="votes_block nstar big pull-right">
												<div class="ratings">
													<div class="inner_rating">
														<?for($i=1;$i<=5;$i++):?>
															<div class="item-rating <?=(round($ratingValue) >= $i ? "filed" : "");?>"><?=CMShop::showIconSvg("star", SITE_TEMPLATE_PATH."/images/svg/star.svg");?></div>
														<?endfor;?>
													</div>
												</div>
											</div>
								...
								<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
									<div class="body-info">
										<?if(!$notSlider):?>
											<?=CMShop::showIconSvg("quote muted ncolor", SITE_TEMPLATE_PATH."/images/svg/quote.svg", "", "", true, false);?>
										<?endif;?>
										<?if(in_array('RATING', $arParams['PROPERTY_CODE'])):?>
											<?$ratingValue = ($arItem['DISPLAY_PROPERTIES']['RATING']['VALUE'] ? $arItem['DISPLAY_PROPERTIES']['RATING']['VALUE'] : 0);?>
										...
										<?if(($arParams['PREVIEW_TRUNCATE_LEN'] > 0) && (strlen($arItem['~PREVIEW_TEXT']) > $arParams['PREVIEW_TRUNCATE_LEN'])):?>
											<div class="link-block-more">
												<span><span class="button transparent btn-xs dyn-jsform" data-event="jqm" data-param-form_id="<?=$arItem['ID'];?>" data-param-type="review" data-name="review-detail"><?=Loc::getMessage('MORE_REVIEWS');?></span></span>
											</div>
										<?endif;?>
												...
												<div class="row flexbox">
													<?foreach((array)$arItem['DISPLAY_PROPERTIES']['FILE']['VALUE'] as $arFileItem):?>
														<div class="col-md-6 col-sm-6 col-xs-12">
															<?$arFile=CMShop::GetFileInfo($arFileItem);?>

...
if($arParams["NOT_SLIDER"] == "Y" && $arResult['ITEMS']){
	...
	foreach($arResult['ITEMS'] as $key => $arItem){
		...
		if($arStaffsID && $nStaffIblockID){
			$arResult['STAFF'] = CMShopCache::CIblockElement_GetList(array('CACHE' => array('TAG' => CMShopCache::GetIBlockCacheTag($nStaffIblockID), 'MULTI' => 'N', 'GROUP' => 'ID')), array('IBLOCK_ID' => $nStaffIblockID, 'ID' => $arStaffsID), false, false, array('ID', 'NAME', 'PROPERTY_POST', 'PREVIEW_PICTURE'));
		}
	}

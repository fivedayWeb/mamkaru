	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			...
			foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arAddToBasketData = CMShop::GetAddToBasketArray($arOffer, 1, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);
		...
		{
			$arItem["FIX_PRICE_MATRIX"] = CMShop::checkPriceRangeExt($arItem);
		}

		//format prices when USE_PRICE_COUNT
		$arItem = array_merge($arItem, CMShop::formatPriceMatrix($arItem));

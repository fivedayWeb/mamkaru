...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		if($arResult['MORE_PHOTO']){
			foreach($arResult['MORE_PHOTO'] as $i => $arImage){
				if(($arImage["ID"] && !$arOffer['DETAIL_PICTURE']) || ($arImage["ID"] && $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y')){
	...
	$postfix = '';
	global $arSite;
	if(\Bitrix\Main\Config\Option::get("aspro.mshop", "HIDE_SITE_NAME_TITLE", "N")=="N")
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			{	
				...
				$arOffer = array_merge($arOffer, CMShop::formatPriceMatrix($arOffer));
				$sPriceMatrix = CMShop::showPriceMatrix($arOffer, $arParams, $arOffer['~CATALOG_MEASURE_NAME']);
			}
			...
			$arOneRow = array(
				...
				'ARTICLE_SKU' => ($arParams['SHOW_ARTICLE_SKU'] == 'Y' ? (isset($arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']) ? '<span class="block_title" itemprop="name">'.$arResult['PROPERTIES']['CML2_ARTICLE']['NAME'].': '.'</span><span class="value" itemprop="value">'.$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'].'</span>' : '') : ''),
				...
				'AVAILIABLE' => CMShop::GetQuantityArray($totalCount, array(), "Y"),
	...
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			{				
				...
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CMShop::formatPriceMatrix($arOffer));
			}
		}
	...
	if($arParams['USE_PRICE_COUNT'] == 'Y')
	{
		if($arResult['OFFERS'])
		{
			foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOffer["FIX_PRICE_MATRIX"] = CMShop::checkPriceRangeExt($arOffer);
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CMShop::formatPriceMatrix($arOffer));
			}
			$arResult['MIN_PRICE'] = CMShop::getMinPriceFromOffersExt(
				$arResult['OFFERS'],
		...
		{
			$arResult["FIX_PRICE_MATRIX"] = CMShop::checkPriceRangeExt($arResult);
		}			
	}
	
	//format prices when USE_PRICE_COUNT
	$arResult = array_merge($arResult, CMShop::formatPriceMatrix($arResult));
}

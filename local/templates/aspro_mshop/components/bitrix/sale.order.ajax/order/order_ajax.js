...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			//poulhouse
			var deliverydesc = BX.create('p', {props: {className: 'bx-soa-pp-desc'}}), addedHtml = '';
				addedHtml += 'Подробные условия доставки вы можете узнать по ';
				addedHtml += '<a href="/delivery/">ссылке</a>';

			deliverydesc.innerHTML += addedHtml;
			deliveryItemsContainer.appendChild(deliverydesc);
		},
		...
		createDeliveryItem: function(item)
		{
			...
			{
				//poulhouse
				if(deliveryId == 3) item.PRICE_FORMATED = '600 руб';
				if(deliveryId == 28) item.PRICE_FORMATED = '300 руб';
				if(deliveryId == 25) item.PRICE_FORMATED = '300 руб';
				if(deliveryId == 30) item.PRICE_FORMATED = '600 руб';  //Таганрог-Чалтырь-Новочекасск-Шахты менее 7000 руб.				
				if(deliveryId == 33) item.PRICE_FORMATED = '600 руб';  //Аксай-Батайск менее 7000 руб.
				if(deliveryId == 32) item.PRICE_FORMATED = '300 руб';  //Аксай-Батайск более 7000 руб.
				if(deliveryId == 34) item.PRICE_FORMATED = 'от 500 руб';  //остальные регионы
			...
			else if (item.PRICE < 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined')
			{
				if(deliveryId == 3) item.PRICE_FORMATED = '600 руб';
				if(deliveryId == 28) item.PRICE_FORMATED = '300 руб';
				if(deliveryId == 25) item.PRICE_FORMATED = '300 руб';
		...
		editTotalBlock: function()
		{
			...
			this.result.TOTAL.ORDER_TOTAL_PRICE = this.result.TOTAL.ORDER_PRICE;
			console.log('total.ORDER_PRICE = '+total.ORDER_PRICE);
			...
			console.log('total.DELIVERY_PRICE = '+total.DELIVERY_PRICE);
			...
			if (this.result.DELIVERY.length)
			{
				...
				this.totalInfoBlockNode.appendChild(
					BX.create('DIV',{
							...
							html: this.getSelectedDelivery().DESCRIPTION
						}
					)
				);
			...
			{
				console.log('total.ORDER_TOTAL_PRICE = '+total.ORDER_TOTAL_PRICE);
			...
			{
				console.log('total.PAY_SYSTEM_PRICE = '+total.PAY_SYSTEM_PRICE);
		...
		createTotalUnit: function(name, value, params) {
			...
			if (params.error) {
				...
				console.log('params.error ', totalValue);
			} else if (params.free) { 
				//poulhouse--change delivery price
				if(this.getSelectedDelivery().ID == 3) value = '600 руб';
				if(this.getSelectedDelivery().ID == 28) value = '300 руб';
				if(this.getSelectedDelivery().ID == 25) value = '300 руб';
				if(this.getSelectedDelivery().ID == 30) value = '600 руб';  //Таганрог-Чалтырь-Новочекасск-Шахты менее 7000 руб.
				if(this.getSelectedDelivery().ID == 33) value = '600 руб';  //Аксай-Батайск менее 7000 руб.
				if(this.getSelectedDelivery().ID == 32) value = '300 руб';  //Аксай-Батайск более 7000 руб.
				if(this.getSelectedDelivery().ID == 34) value = 'от 500 руб';  //остальные регионы
				
				totalValue = [BX.create('SPAN', {
					props: {className: 'bx-soa-price-free'},
					html: value //poulhouse--*/ this.getSelectedDelivery().DESCRIPTION //'Рассчитывается менеджером'
				})];
				console.log('params.free ', this.getSelectedDelivery().ID );
			} else {
				totalValue = [value];

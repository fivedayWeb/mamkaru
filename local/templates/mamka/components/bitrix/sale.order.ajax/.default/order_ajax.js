...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            //custom
            this.basketItems = parameters.basketItems;
        ...
        sendRequest: function (action, actionData) {
            ...
            $.ajax({
                ...
                data: {ITEMS: this.basketItems, USERDATA: this.userData},
                method: 'POST',
                success: BX.proxy(function (response) {
                    if (response) {
                        debugger;
                        this.showError(this.mainErrorsNode, response);
            ...
            for (i = 0; i < allSections.length; i++) {
                if (allSections[i].id == actionSection.id && allSections[i + 1]) {
                    ...
                    if (this.params.SKIP_USELESS_BLOCK == 'Y') {
                        ...
                        if (nextSection.id == this.paySystemBlockNode.id || nextSection.id == this.pickUpBlockNode.id) {
                            onePaySystem = this.result.PAY_SYSTEM
                                && this.result.PAY_SYSTEM.length == 1
                                && this.result.PAY_FROM_ACCOUNT != 'Y';
                        }

                        if (nextSection.id == this.pickUpBlockNode.id && onePickUp && !onePaySystem) {
        ...
        showActualBlock: function () {
            ...
            while (allSections[i]) {
                ...
                if (allSections[i].id == this.propsBlockNode.id) this.isValidPropertiesBlock(false, true);
        ...
        editOrder: function () {
            ...
            var _this = this;
            var propsDelivery = this.result.ORDER_PROP.properties.filter(function (item) {
                return _this.propertyGroupIsDelivery(item.PROPS_GROUP_ID);
            });
            if (propsDelivery.length > 0) {
                this.regionBlockNode.style.display = '';
            } else {
                this.regionBlockNode.style.display = 'none';
            }

            var selectedPersonType = Object.values(this.result.PERSON_TYPE).filter(function (item) {
                return item.CHECKED == 'Y';
            })[0];
            if (selectedPersonType.ID == 2) {
                this.paySystemBlockNode.style.display = 'none';
            } else {
                this.paySystemBlockNode.style.display = '';
            }
        ...
        createBasketItemContent: function (data) {
            ...
            if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length){
				...
				if (data.PRODUCT_ID == 602503) {
					titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + '</a><p>+ Пустышка Bibs - 0 ₽</p>';
				}
			}
            ...
            return BX.create('DIV', {
                ...
                children: propsNodes.length ? [
                    ...
                    BX.create('DIV', {props: {className: 'bx-soa-item-title'}, html: titleHtml})//, children: akciyaHtml ? {BX.create('p', {html:akciyaHtml} )} })
                ]
			});
        },
        ...
        editActiveRegionBlock: function (activeNodeMode) {
            ...
            if (this.initialized.region) {
                ...
                this.editPropsItems(regionNode, false);
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                var propsNodeCol = BX.create('DIV', {props: {className: 'col-xs-12'}});
                this.getPersonTypeControl(propsNodeCol);
                propsNode.appendChild(propsNodeCol);

                this.editPropsItems(propsNode, true);
                ...
                if (this.propsBlockNode.getAttribute('data-visited') == 'true') {
                    validationErrors = this.isValidPropertiesBlock(false, true);
        ...
        editFadePropsContent: function (node) {
            ...
            if (this.propsBlockNode.getAttribute('data-visited') == 'true') {
                validPropsErrors = this.isValidPropertiesBlock(false, true);
        ...
        propertyGroupIsDelivery: function (groupID) {
            return groupID == "1" || groupID == "4";
        },

        editPropsItems: function (propsNode, skipDelivery) {
            ...
            while (group = groupIterator()) {
                ...
                while (property = propsIterator()) {
                    ...
                    if (skipDelivery == this.propertyGroupIsDelivery(property.getGroupId())) continue;
            ...
            if (textNode) {
                ...
                if (settings.PATTERN && settings.PATTERN.length) {
                    if (settings.MULTIPLE == 'Y') {
                        ...
                        textNode.setAttribute('pattern', settings.PATTERN.substr(1, settings.PATTERN.length - 3));
                    }
                }
            }
        ...
        isValidForm: function () {
            ...
            var regionErrors = this.isValidRegionBlock(),
                propsErrors = this.isValidPropertiesBlock(false, true),
        ...
        isValidRegionBlock: function () {
            ...
            var propsError = this.isValidPropertiesBlock(false, false);
            if (propsError.length) {
                if (!regionErrors.length) {
                    regionErrors = propsError;
                } else {
                    regionErrors = regionErrors.concat(propsError);
                }
            }
        ...
        isValidPropertiesBlock: function (excludeLocation, skipDelivery) {
            if (!this.options.propertyValidation)
                return [];
            ...
            for (i = 0; i < props.length; i++) {
                var propID = $(props[i]).attr('data-property-id-row');
                var prop = this.result.ORDER_PROP.properties.filter(function (item) {
                    return item.ID == propID
                })[0];
                if (prop == undefined || skipDelivery == this.propertyGroupIsDelivery(prop.PROPS_GROUP_ID)) continue;

...
function updateBasketTable(basketItemId, res)
{
	...
	if (!!res.BASKET_DATA)
	{
		var total_new_sum = res.BASKET_DATA.allSum*1,
			total_old_sum = res.BASKET_DATA.DISCOUNT_PRICE_ALL*1 + res.BASKET_DATA.allSum*1,
			total_discount_percentage = Math.round(res.BASKET_DATA.DISCOUNT_PRICE_ALL/(res.BASKET_DATA.DISCOUNT_PRICE_ALL*1 + res.BASKET_DATA.allSum*1)*100);
		
		$('#total_new_price').html(total_new_sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")+' ₽');
		$('#total_percentage_discount').html(total_discount_percentage + "%");

		$('#total_percentage_discount').show();
		$('.cart-footer .discount-title').show();
        if(total_discount_percentage != 0) {
            $('#total_old_price').html('<s>' + total_old_sum + ' ₽</s>');
        }
        else{
            $('#total_old_price').html( total_old_sum + ' ₽');
		}
		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				var htmlItem = $('#basket_items_list tr#'+id);
				$('.cart-price',htmlItem).html(item.FULL_PRICE.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")  + ' ₽');
				$('.cart-new-price',htmlItem).html(item.PRICE.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ") + ' ₽');
				$('.count-counter',htmlItem).html(item.QUANTITY);
...
function recalcBasketAjax(params)
{
	...
	BX.ajax({
		...
		onsuccess: function(result)
		{
			...
			var sumQuantity = 0;
			$('.count-counter').each(function(i, item) {
				sumQuantity += parseInt($(item).text());
			});
			updateBasket(sumQuantity);
		}
	});
}
...
$(function() {
	BX.ready(function () {
		...
		var sku_props = BX.findChildren(BX('cart-table'), {tagName: 'li', className: 'sku_prop'}, true),
			i,
			couponBlock;
		if (!!sku_props && sku_props.length > 0) {
			for (i = 0; sku_props.length > i; i++) {

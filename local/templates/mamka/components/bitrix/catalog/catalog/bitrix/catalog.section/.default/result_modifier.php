...
$excludeSections = [
    'aksessuary_dlya_detskoy_komnaty',
];
...
$db_nav = CIBlockSection::GetNavChain(false, $res['ID']);
while ($nav = $db_nav->fetch()) {
    if (in_array($nav['CODE'], $skuStyleSections)) {
        $is_sku = true;
    } elseif (in_array($nav['CODE'], $excludeSections)) {
        $is_sku = false;
        break;
    }
}
...
$arResult['CONVERTED_ITEMS'] = array_map(
    function ($item) {
        return \Custom\CatalogItem::convertFromCatalogSection($item);
    },
    $arResult['ITEMS']
);
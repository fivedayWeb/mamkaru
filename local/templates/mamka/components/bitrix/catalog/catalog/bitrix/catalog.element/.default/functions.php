...
if (!function_exists('prepareOfferNameFromByProduct'))
{
    function prepareOfferNameFromByProduct($arResult, $arOffer){
        $tmp = str_replace(['(',')',$arResult['NAME']],'',$arOffer['NAME']);
        $tmp = str_replace('+',' ',$tmp);
        preg_match('#[0-9]+([A-я|]+)\s([A-я|]+)#',$tmp,$matches);

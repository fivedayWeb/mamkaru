...
if ('' != $arParams['TEMPLATE_THEME']) {
    ...
    if ('site' == $arParams['TEMPLATE_THEME']) {
        ...
        $arParams['TEMPLATE_THEME'] = COption::GetOptionString('main', 'wizard_' . $templateId . '_theme_id', 'blue',
            SITE_ID);
    }
    if ('' != $arParams['TEMPLATE_THEME']) {
...
$arParams['ADD_TO_BASKET_ACTION'] = array_filter($arParams['ADD_TO_BASKET_ACTION'],
    'CIBlockParameters::checkParamValues');
if (empty($arParams['ADD_TO_BASKET_ACTION']) || (!in_array('ADD', $arParams['ADD_TO_BASKET_ACTION']) && !in_array('BUY',
...
$strEmptyPreview = ConfigOptionsClass::$NO_IMAGE;
...
$productSlider = CIBlockPriceTools::getSliderForItem($arResult, $arParams['ADD_PICT_PROP'],
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
    ...
    CIBlockPriceTools::getTreePropertyValues($arSKUPropList, $arNeedValues); //Тут появляются значения
    $arSKUPropIDs = array_keys($arSKUPropList);
    ...
    foreach ($arResult['OFFERS'] as $keyOffer => $arOffer) {
        ...
        $offerSlider = CIBlockPriceTools::getSliderForItem($arOffer, $arParams['OFFER_ADD_PICT_PROP'],
...
$arImportSettingsRules = unserialize(file_get_contents(\OneCImportMod::getFilePath()));
$arPropIds = [];
array_walk_recursive($arImportSettingsRules, function ($value, $key) use (&$arPropIds) {
    if ($key == 'PROP_ID') {
        $arPropIds[] = $value;
    }
});
$arPropsIds = array_unique($arPropIds);

$arPropCodesImportSettings = [];
$resProp = CIBlockProperty::GetList(
    [],
    [
        'IBLOCK_ID' => \Bitrix\Main\Config\Option::get("grishchenk.onecimportmod", "iblock_id", "")
    ]
);
while ($arProp = $resProp->Fetch()) {
    if (in_array($arProp['ID'], $arPropsIds)) {
        $arPropCodesImportSettings[] = $arProp['CODE'];
    }
}

$arResult['SKIP_PROPERTY'] = array_merge(
    $arResult['SKIP_PROPERTY'],
    $arParams['OFFERS_PROPERTY_CODE'],
    $arPropCodesImportSettings
);

$arParams['TOP_BLOCK_PROPERTIES'] = [
    'CML2_ARTICLE',
    'ARTNUMBER',
    'CML2_MANUFACTURER',
    'SEASONAL',
...
if ($arResult['CATALOG_TYPE'] == 3) {
    ...
    if (!$arResult['OFFER_ID_SELECTED']) {
        $arResult['OFFER_SELECTED'] = $arResult['OFFERS'][0];
    }//TODO проверить при отсутвии предложений

    foreach ($arResult['OFFERS'] as &$arOffer) {
        if ($arOffer['ID'] == $arResult['OFFER_ID_SELECTED']) {
            $arResult['OFFER_SELECTED'] = $arOffer;
        }
    }
    unset($arOffer);

    $arTmpProperties = [];
    foreach ($arResult['PROPERTIES'] as $k => $arProperty) {
        if (
            (
                !in_array($arProperty['CODE'], $arResult['SKIP_PROPERTY'])
                ||
                in_array($arProperty['CODE'], $arParams['OFFERS_PROPERTY_CODE'])
            )
            && !empty($arProperty['VALUE'])
        ) {
            $arTmpProperties[] = $arProperty;
        }
    }

    $arResult['PROPERTY_SPLIT_BY_OFFER_TOP'] = [];
    $arResult['PROPERTY_SPLIT_BY_OFFER_BOTTOM'] = [];
    foreach ($arResult['OFFERS'] as $arOffer) {
        $filledProp = [];
        $arItemsTop = [];
        $arItemsBottom = [];
        foreach ($arOffer['PROPERTIES'] as $arProperty) {
            if (!in_array($arProperty['CODE'], $arResult['SKIP_PROPERTY'])) {
                if (!empty($arProperty['VALUE'])) {
                    if ($arProperty['PROPERTY_TYPE'] == "E" && !empty($arProperty['VALUE'])) {
                        $arProperty['IBLOCK_VALUES'] = \Bitrix\Saybert\Helpers\IblockElement::getIblockElement($arProperty['VALUE']);
                        $arProperty['DISPLAY_VALUE'] = $arProperty['IBLOCK_VALUES']['NAME'];
                    } else {
                        $arProperty['DISPLAY_VALUE'] = $arProperty['VALUE'];
                    }

                    if (in_array($arProperty['CODE'], $arParams['TOP_BLOCK_PROPERTIES'])) {
                        $arItemsTop[] = $arProperty;
                    } else {
                        $arItemsBottom[] = $arProperty;
                    }

                    $filledProp[] = $arProperty['CODE'];
                }
            }
        }
        foreach ($arTmpProperties as $arProperty) {
            if (!in_array($arProperty['CODE'], $filledProp) || $arProperty['CODE'] == 'ADDITIONAL_STRING_PROPERTY') {
                if ($arProperty['PROPERTY_TYPE'] == "E" && !empty($arProperty['VALUE'])) {
                    $arProperty['IBLOCK_VALUES'] = \Bitrix\Saybert\Helpers\IblockElement::getIblockElement($arProperty['VALUE']);
                    $arProperty['DISPLAY_VALUE'] = $arProperty['IBLOCK_VALUES']['NAME'];
                } else {
                ...
                if ($arProperty['CODE'] == 'BRAND') {
                    $arResult['BRAND'] = $arProperty;
                }
        ...
        usort($arItemsTop, function ($a, $b) {
            return ($a['SORT'] > $b['SORT']) ? 1 : -1;
        });
        usort($arItemsBottom, function ($a, $b) {
            return ($a['SORT'] > $b['SORT']) ? 1 : -1;
        });
    ...
    foreach ($arResult['OFFERS'] as $arOffers) {
        foreach ($arOffers['TREE'] as $propId => $propValue) {
            $arOfferSkuProp[str_replace('PROP_', '', $propId)] = $propValue;
        }
        $arJsOffers[] = [
            'offerID' => $arOffers['ID'],
            'property' => $arOfferSkuProp,
            'canBuy' => $arOffers['CAN_BUY']
        ];
    }

    $arJsSku = [];
    foreach ($arResult['SKU_PROPS'] as $arSkuProp) {
        if (!$arResult['OFFERS_PROP'][$arSkuProp['CODE']]) {
            continue;
        }
        $tmp = [
            'ID' => $arSkuProp['ID'],
        ...
        foreach ($arSkuProp['VALUES'] as $code => $arValue) {
            if ($code == 0) {
                continue;
            }
            $tmp['VALUES'][$code] = $arValue;
        }
        $arJsSku[] = $tmp;
    }

    $arResult['JS_PARAMS']['OFFERS'] = $arJsOffers;
    $arResult['JS_PARAMS']['SKU'] = $arJsSku;
} elseif ($arResult['CATALOG_TYPE'] == 1) {
    $arResult['PROPERTY_SPLIT_BY_OFFER_TOP'] = [];
    $arResult['PROPERTY_SPLIT_BY_OFFER_BOTTOM'] = [];

    foreach ($arResult['PROPERTIES'] as $arProperty) {
        if ((!in_array($arProperty['CODE'], $arResult['SKIP_PROPERTY']) || in_array($arProperty['CODE'], $arParams['OFFERS_PROPERTY_CODE'])) && !empty($arProperty['VALUE'])) {
            ...
            if (in_array($arProperty['CODE'], $arParams['TOP_BLOCK_PROPERTIES'])) {
                $arResult['PROPERTY_SPLIT_BY_OFFER_TOP'][] = $arProperty;
            } else {
                $arResult['PROPERTY_SPLIT_BY_OFFER_BOTTOM'][] = $arProperty;
            }
        }
    }
}

//MORE_PHOTO - в каталоге и в предложениях
$arResult['PHOTOS'] = [];
$arResult['PROPERTIES']['MORE_PHOTO']['VALUE'];
$arResult['OFFERS'][0]['PROPERTIES']['MORE_PHOTO']['VALUE'];
$arTmpPhoto = [];

$arResult['OFFER_SELECTED']['ID'] = \Custom\CatalogItem::getNeedleOffer($arResult);

if ($arResult['OFFER_SELECTED']['ID']) {
    foreach ($arResult['OFFERS'] as $k => $arOffer) {
        if ($arOffer['ID'] == $arResult['OFFER_SELECTED']['ID']) {
            $photoID = $arOffer['DETAIL_PICTURE']['ID'];
            if ($photoID > 0) {
                $arTmpPhoto[] = [
                    'id' => $photoID,
                    'type' => 'product',
...
foreach ($arResult['OFFERS'] as $arOffer) {
    ...
    } elseif (!empty($arOffer['PROPERTIES']['MORE_PHOTO']['VALUE'])) {
        $arTmpPhoto[] = [
            'id' => current($arOffer['PROPERTIES']['MORE_PHOTO']['VALUE']),
            'type' => 'offer',
            'offerId' => $arOffer['ID'],
        ];
    }
...
foreach ($arTmpPhoto as $arPhoto) {
    $arPhoto['detail'] = CFile::GetPath($arPhoto['id']);
    $arPhoto['preview'] = \CFile::ResizeImageGet($arPhoto['id'], $arPhotoSize, BX_RESIZE_IMAGE_PROPORTIONAL)['src'];
    $arPhoto['small'] = \CFile::ResizeImageGet($arPhoto['id'], $arPhotoPreviewSize,
...
if (!$arResult['PHOTOS']) {
    $arResult['PHOTOS'][] = array(
        'detail' => ConfigOptionsClass::$NO_IMAGE,
        'preview' => ConfigOptionsClass::$NO_IMAGE,
        'small' => ConfigOptionsClass::$NO_IMAGE,
    );
}

$arResult['OFFERS_BY_PHOTO'] = [];
...
foreach ($arResult['OFFERS'] as $k => $arOffer) {
    ...
    if (!empty($arOffer['DETAIL_PICTURE'])) {
        ...
        $photoID = current($arOffer['PROPERTIES']['MORE_PHOTO']['VALUE']);
        $hasImage = true;
    } elseif (!empty($arOffer['PREVIEW_PICTURE'])) {
        $photoID = $arOffer['PREVIEW_PICTURE']['ID'];
        $hasImage = true;
    }
    if ($hasImage) {
        $arPhoto = [
            'detail' => CFile::GetPath($photoID),
            'preview' => \CFile::ResizeImageGet($photoID, $arPhotoSize, BX_RESIZE_IMAGE_PROPORTIONAL)['src'],
            'small' => \CFile::ResizeImageGet($photoID, $arPhotoPreviewSize, BX_RESIZE_IMAGE_PROPORTIONAL)['src'],
        ];
    } else {
        $arPhoto = [
            'detail' => ConfigOptionsClass::$NO_IMAGE,
    ...
    if ($arOffer['ID'] == $arResult['OFFER_SELECTED']['ID']) {
        $arResult['CURRENT_PHOTO'] = false;
        if ($hasImage) {
            $arResult['CURRENT_PHOTO'] = [
                'detail' => CFile::GetPath($photoID),
                'preview' => \CFile::ResizeImageGet($photoID, $arPhotoSize, BX_RESIZE_IMAGE_PROPORTIONAL)['src'],
                'small' => \CFile::ResizeImageGet($photoID, $arPhotoPreviewSize, BX_RESIZE_IMAGE_PROPORTIONAL)['src'],
            ];
    ...
    $arResult['OFFERS_BY_PHOTO'][] = [
        'ID' => $arOffer['ID'],
        'NAME' => $arOffer['NAME'] ?: $arResult['NAME'],
        'PREPARED_NAME' => prepareOfferNameFromByProduct($arResult, $arOffer),
        'CURRENT' => $arOffer['ID'] == $arResult['OFFER_SELECTED']['ID'],

        'detail' => $arPhoto['detail'],
        'preview' => $arPhoto['preview'],
        'small' => $arPhoto['small'],

        'PROPERTIES' => $arOffer['DISPLAY_PROPERTIES'],
        'CAN_BUY' => $arOffer['CAN_BUY'],
    ];
}
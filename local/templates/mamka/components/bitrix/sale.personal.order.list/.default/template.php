...
use Bitrix\Main, \Bitrix\Saybert\Helpers\CatalogElement, \Bitrix\Saybert\Helpers\IblockElement, \Bitrix\Saybert\Catalog; ?>

<? if (!empty($arResult['ORDERS'])): ?>
    <? foreach ($arResult['ORDERS'] as $arOrder): ?>
        <div class="history-order clear">
            <div class="history-order-title">
                <? $dateTime = false;
                if ($arOrder['ORDER']['DATE_INSERT'] instanceof \Bitrix\Main\Type\DateTime) {
                    $dateTime = FormatDate($arParams['ACTIVE_DATE_FORMAT'],
                        $arOrder['ORDER']['DATE_INSERT']->getTimestamp());
                }
                ?>
                Заказ <?= $arOrder['ORDER']['ID'] ?>
                <? if ($dateTime): ?>от <?= $dateTime ?><? endif; ?>
            </div>
            <? foreach ($arOrder['BASKET_ITEMS'] as $basketItemID => $arBasketItem): ?>
                <?
                $arProduct = IblockElement::getIblockElement($arBasketItem['PRODUCT_ID']);
                ?>
                <div class="history-order-product clear">
                    <? if ($arProduct): ?>
                        <div class="history-order-product-image">
                            <? if ($arProduct['PREVIEW_PICTURE']): ?>
                                <? $picture = CFile::ResizeImageGet($arProduct['PREVIEW_PICTURE'],
                                    array('width' => 120, 'height' => 120), BX_RESIZE_IMAGE_PROPORTIONAL); ?>
                                <img src="<?= $picture['src'] ?>">
                            <? else: ?>
                                <img src="<?= SITE_TEMPLATE_PATH ?>/images/no_photo.jpg">
                            <? endif; ?>
                        </div>
                        <div class="history-order-product-info">
                            <a href="<?= $arBasketItem['DETAIL_PAGE_URL'] ?>"
                               class="history-order-product-name"><?= $arBasketItem['NAME'] ?>
                                - <?= $arBasketItem['QUANTITY'] ?> шт.</a>
                            <? if ($arProduct['PROPERTIES']['ARTNUMBER']['VALUE']): ?>
                                <div class="history-order-product-code">
                                    Артикул
                                    <?= $arProduct['PROPERTIES']['ARTNUMBER']['VALUE'] ?>
                                </div>
                            <? endif; ?>
                            <div class="history-order-product-price">
                                <?= Catalog\Tools::formatPrice($arBasketItem['PRICE']) ?>
                                <? if ($arBasketItem['CURRENCY'] == 'RUB'): ?>₽.<? else: ?><?= $arBasketItem['CURRENCY'] ?><? endif; ?>
                            </div>
                            <a href="<?= $arBasketItem['DETAIL_PAGE_URL'] ?>" class="history-order-product-add-review">
                                Оставить отзыв
                            </a>
                        </div>
                    <? else: ?>
                        ...
                        <div class="history-order-product-info">
                            <a class="history-order-product-name"><?= $arBasketItem['NAME'] ?>
                                - <?= $arBasketItem['QUANTITY'] ?> шт.</a>
                            <div class="history-order-product-price">
                                <?= Catalog\Tools::formatPrice($arBasketItem['PRICE']) ?>
                                <? if ($arBasketItem['CURRENCY'] == 'RUB'): ?>₽.<? else: ?><?= $arBasketItem['CURRENCY'] ?><? endif; ?>
            ...
            <? endforeach; ?>
            <div class="history-order-check">
                <?
                $arStatus = $arResult['INFO']['STATUS'][$arOrder['ORDER']['STATUS_ID']];
                ?>
                <div class="history-order-check-status">Статус: <strong><?= $arStatus['NAME'] ?></strong></div>
                <div class="history-order-check-delivery">
                    Способ получения:
                    <? foreach ($arOrder['SHIPMENT'] as $arShipment): ?>
                        <strong><?= $arShipment['DELIVERY_NAME'] ?></strong>
                    <? endforeach; ?>
                </div>
                <div class="history-order-check-summ">
                    Итого:
                    <strong>
                        <?= Catalog\Tools::formatPrice($arOrder['ORDER']['PRICE']) ?>
                        <? if ($arOrder['ORDER']['CURRENCY'] == 'RUB'): ?>₽<? else: ?><?= $arOrder['ORDER']['CURRENCY'] ?><? endif; ?>
                    </strong>
                </div>
                <? foreach ($arOrder['PAYMENT'] as $key => $arPayment): ?>
                    <? if (!in_array($arPayment['PAY_SYSTEM_ID'], array(9))) {
                        continue;
                    } ?>
                    <? if ($arPayment['PAID'] == 'N'): ?>
                        <?
                        if ($arOrder['ORDER']['PROPS']['PROPERTY_VAL_BY_CODE_SEND_PAYMENT_LINK'] == 'Y'):
                            ?>
                            <div class="pay-item-block">
                                <a href="<?= $arPayment['PSA_ACTION_FILE'] ?>"
                                   class="pay-button"<? if ($arPayment['NEW_WINDOW'] == 'Y'):?> target="_blank"<? endif ?>>Оплатить</a>
                                <span><?= $arPayment['PAY_SYSTEM_NAME'] ?></span>
                            </div>
                        <? endif; ?>
                    <? endif ?>
                <? endforeach ?>
